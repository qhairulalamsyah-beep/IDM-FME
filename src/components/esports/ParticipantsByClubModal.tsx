...
interface ParticipantsByClubModalProps {
  isOpen: boolean;
...
export function ParticipantsByClubModal({
  isOpen,
  onOpenChange,
}: ParticipantsByClubModalProps) {
  ...
  const fetchAllData = useCallback(async () => {
    setLoading(true);
    try {
      // Fetch all users with their clubs
      ...
      // Sort clubs by total member points
  ...
  const expandAll = () => {
    setExpandedClubs(new Set(clubs.map(c => c.id)));
  };
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="relative flex-shrink-0 p-5 pb-4 border-b border-white/[0.06]">
              {/* Glow line */}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button
                    onClick={expandAll}
                    className="px-2.5 py-1 rounded-lg text-[10px] font-medium text-white/30 hover:text-white/50 transition-colors"
                  >
                    Buka Semua
                  </button>
            ...
            {/* Footer */}
            <div className="flex-shrink-0 p-4 border-t border-white/[0.06] bg-[#0E0E12]">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => onOpenChange(false)}
                  className="px-5 py-2 rounded-xl bg-white/[0.06] border border-white/[0.08] text-[12px] font-semibold text-white/60 hover:bg-white/10 transition-colors"
                >
                  Tutup
                </button>
              </div>

...
export function TournamentTab({
  ...
  useEffect(() => {
    ...
    // Don't lookup if name is too short — clear auto-load via ref-synced state
    ...
    lookupTimerRef.current = setTimeout(() => {
      setIsLookingUp(true);
      // Search in already-loaded users (no API call needed)
  ...
  const handleAvatarUpload = useCallback((url: string) => {
    ...
    setIsAutoLoaded(false); // Once user uploads manually, no longer auto-loaded
  }, []);
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-white/30 font-medium">Isi data dan upload foto karakter untuk bergabung</p>
                  </div>
                </div>
                ...
                <div>
                  <label className="flex items-center gap-1.5 text-[11px] text-white/40 mb-2 uppercase tracking-wider font-semibold">
                    <Camera className={`w-3 h-3 ${isMale ? 'text-amber-400' : 'text-violet-400'}`} />

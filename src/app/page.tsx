...
export default function IDOLMETAApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/clubs?gender=${division}&limit=10`, { signal: controller.signal });
        if (res.ok) {
          const data = await res.json();
  ...
  useEffect(() => {
    ...
    const handler = () => {
      fetch(`/api/clubs?gender=${division}&limit=10`)
  ...
  const historyTournaments = useMemo(() => tournaments.map(t => ({
    ...t,
    _count: t._count || { registrations: 0, teams: 0, matches: 0 },
  })), [tournaments]);

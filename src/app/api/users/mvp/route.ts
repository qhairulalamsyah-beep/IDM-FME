...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    // Remove MVP flag, deduct the fixed bonus (clamped to 0 minimum)
    await db.$transaction([
      db.user.update({
        where: { id: userId },
        data: { isMVP: false, mvpScore: 0 },
      }),
      db.$executeRaw`UPDATE "User" SET points = MAX(0, points - ${MVP_BONUS_POINTS}) WHERE id = ${userId}`,
    ]);
    await db.$transaction([
      db.ranking.upsert({
        where: { userId },
      ...
      db.$executeRaw`UPDATE "Ranking" SET points = MAX(0, points - ${MVP_BONUS_POINTS}) WHERE "userId" = ${userId}`,
    ]);
